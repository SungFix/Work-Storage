...
let db=null,auth=null,unsubscribeCloud=null,saveTimer=null;
const state={user:null,items:[],folders:[],activeId:null,filter:'home',search:'',viewMode:localStorage.getItem(VIEW_KEY)||'cards',drawerMax:false,syncing:false};
...
function normalizeItem(item,id){
  return{
    ...
    blocks:Array.isArray(item.blocks)&&item.blocks.length?item.blocks:[{...blankBlock('text'),text:item.content||''}]
  };
}
...
function searchable(item){
  return [item.title,(item.tags||[]).join(' '),...(item.blocks||[]).flatMap(b=>Object.values(b).flatMap(v=>Array.isArray(v)?v.map(x=>typeof x==='object'?Object.values(x).join(' '):x):typeof v==='object'?'':v))].join(' ').toLowerCase();
}
...
function setScreen(name){$('#authView')?.classList.toggle('hidden',name!=='auth');$('#appView')?.classList.toggle('hidden',name!=='app')}
...
async function initFirebase(){
  try{
    ...
    auth.onAuthStateChanged(async user=>{
      ...
      if(!user){stopSync();state.items=[];state.folders=[];state.activeId=null;setScreen('auth');return}
      updateAccount();setScreen('app');setSync('saving','Carregando...');
      await loadWorkspace();subscribeWorkspace();renderAll();setSync('cloud','Sincronizado');
    });
  }catch(err){console.error(err);setScreen('auth');alert('Não foi possível iniciar o Firebase. Recarregue a página.')}
}
function stopSync(){if(unsubscribeCloud){unsubscribeCloud();unsubscribeCloud=null}clearTimeout(saveTimer)}

function parseDoc(doc){
  const raw=doc.data();
  if(doc.id==='__security__')return null;
  if((raw.kind==='item'||raw.kind==='folder')&&raw.value){return{kind:raw.kind,value:raw.value,id:doc.id,legacy:false}}
  if(raw.plain&&raw.value){return{kind:raw.kind||'item',value:raw.value,id:doc.id,legacy:false}}
  if(raw.iv&&raw.data)return{kind:'encrypted',value:null,id:doc.id,legacy:false};
  if(raw.title!==undefined||raw.content!==undefined){return{kind:'item',value:normalizeItem(raw,doc.id),id:doc.id,legacy:true}}
  return null;
}
async function loadWorkspace(){
  const snap=await userCollection().get(),items=[],folders=[],legacy=[];let encrypted=0;
  for(const d of snap.docs){
    const parsed=parseDoc(d);if(!parsed)continue;
    if(parsed.kind==='encrypted'){encrypted++;continue}
    if(parsed.kind==='item')items.push(normalizeItem(parsed.value,d.id));
    if(parsed.kind==='folder')folders.push({...parsed.value,id:d.id});
    if(parsed.legacy)legacy.push({id:d.id,kind:parsed.kind,value:parsed.value});
  }
  if(!items.length&&!folders.length){
    try{
      const local=JSON.parse(localStorage.getItem('ws_notebook_v1')||'null');
      if(Array.isArray(local)&&local.length){local.forEach(n=>items.push(normalizeItem(n,n.id)));for(const item of items)legacy.push({id:item.id,kind:'item',value:item})}
    }catch{}
  }
  state.items=items;state.folders=folders;normalizeOrders();
  if(legacy.length)await migrateLegacy(legacy);
  ['ws_notebook_v1','ws_items_v2','ws_items'].forEach(k=>localStorage.removeItem(k));
  if(encrypted)toast('Há registros antigos criptografados que esta versão não consegue abrir.','error');
}
async function migrateLegacy(records){
  setSync('saving','Atualizando dados...');state.syncing=true;
  try{
    for(let i=0;i<records.length;i+=350){
      const batch=db.batch();
      for(const r of records.slice(i,i+350))batch.set(userCollection().doc(r.id),{kind:r.kind,version:4,value:r.value,updatedAt:r.value.updatedAt||now()});
      await batch.commit();
    }
...
function subscribeWorkspace(){
  ...
  unsubscribeCloud=userCollection().onSnapshot(snap=>{
    ...
    const items=[],folders=[];
    for(const d of snap.docs){const p=parseDoc(d);if(!p||p.kind==='encrypted')continue;if(p.kind==='item')items.push(normalizeItem(p.value,d.id));if(p.kind==='folder')folders.push({...p.value,id:d.id})}
    state.items=items;state.folders=folders;normalizeOrders();if(state.activeId&&!state.items.some(i=>i.id===state.activeId))closeDrawer();renderAll();setSync('cloud','Sincronizado');
...
async function saveRecord(kind,obj){
  ...
  try{await userCollection().doc(obj.id).set({kind,version:4,value:obj,updatedAt:obj.updatedAt||now()});setSync('cloud','Sincronizado')}
  finally{state.syncing=false}
}
...
function renderSidebar(){
  const nav=$('#folderNav');if(!nav)return;
  nav.innerHTML=state.folders.map(f=>`<div class="folder-row" draggable="true" data-folder-id="${esc(f.id)}"><button data-filter="folder:${esc(f.id)}"><span>▱</span><span>${esc(f.name)}</span></button><button class="folder-more" data-folder-menu="${esc(f.id)}" aria-label="Opções da pasta">•••</button></div>`).join('');
...
async function exportBackup(){downloadJson({product:'Work Storage',version:4,exportedAt:new Date().toISOString(),items:state.items,folders:state.folders},`work-storage-backup-${new Date().toISOString().slice(0,10)}.json`);toast('Backup exportado.')}
async function importBackup(file){
  try{
    const backup=JSON.parse(await file.text());if(!Array.isArray(backup.items)||!Array.isArray(backup.folders))throw new Error('Formato incompatível');
    if(!confirm('Restaurar este backup? Os itens e pastas atuais serão substituídos.'))return;
    state.syncing=true;const col=userCollection(),current=await col.get();
    for(let i=0;i<current.docs.length;i+=350){const batch=db.batch();current.docs.slice(i,i+350).forEach(d=>batch.delete(d.ref));await batch.commit()}
    const records=[...backup.items.map(x=>({kind:'item',value:normalizeItem(x,x.id)})),...backup.folders.map(x=>({kind:'folder',value:x}))];
    for(let i=0;i<records.length;i+=350){const batch=db.batch();for(const r of records.slice(i,i+350))batch.set(col.doc(r.value.id),{kind:r.kind,version:4,value:r.value,updatedAt:r.value.updatedAt||now()});await batch.commit()}
    state.syncing=false;await loadWorkspace();renderAll();toast('Backup restaurado.')
...
document.addEventListener('click',e=>{
  ...
  if(!e.target.closest('#createMenu')&&!e.target.closest('[data-open-create]')&&!e.target.closest('#newItemBtn')&&!e.target.closest('#quickNewBtn'))toggleCreateMenu(false);
...
$('#signOutBtn').onclick=()=>auth?.signOut();$('#exportBtn').onclick=exportBackup;$('#importBtn').onclick=()=>$('#importInput').click();$('#importInput').addEventListener('change',async e=>{if(e.target.files?.[0])await importBackup(e.target.files[0]);e.target.value=''});
...
$('#blocks').addEventListener('click',async e=>{
  const box=e.target.closest('[data-block-id]'),block=box&&findBlock(box.dataset.blockId),item=activeItem();if(!block||!item)return;
  if(e.target.closest('[data-remove-block]')){item.blocks=item.blocks.filter(b=>b.id!==block.id);touchItem(item);renderDrawer();return}
  if(e.target.closest('[data-reveal-password]')){e.preventDefault();const input=box.querySelector('[data-block-field="password"]');input.type=input.type==='password'?'text':'password';e.target.textContent=input.type==='password'?'Mostrar':'Ocultar';return}
  if(e.target.closest('[data-copy-password]')){e.preventDefault();await navigator.clipboard.writeText(block.password||'');toast('Senha copiada.');return}
  if(e.target.closest('[data-open-link]')){e.preventDefault();if(block.url)window.open(/^https?:\/\//i.test(block.url)?block.url:`https://${block.url}`,'_blank','noopener');return}
  if(e.target.closest('[data-copy-code]')){e.preventDefault();await navigator.clipboard.writeText(block.code||'');toast('Conteúdo copiado.');return}
  const check=e.target.closest('[data-check-id]');if(check&&e.target.closest('[data-remove-check]')){e.preventDefault();block.items=block.items.filter(x=>x.id!==check.dataset.checkId);touchItem(item);renderDrawer();return}
  if(e.target.closest('[data-add-check]')){e.preventDefault();block.items.push({id:makeId('c'),text:'',done:false});touchItem(item);renderDrawer();return}
  const custom=e.target.closest('[data-field-row]');if(custom&&e.target.closest('[data-remove-field]')){e.preventDefault();block.rows=block.rows.filter(x=>x.id!==custom.dataset.fieldRow);touchItem(item);renderDrawer();return}
  if(e.target.closest('[data-add-field]')){e.preventDefault();block.rows.push({id:makeId('f'),key:'',value:''});touchItem(item);renderDrawer();return}
  if(e.target.closest('[data-download-file]')){e.preventDefault();const a=document.createElement('a');a.href=block.dataUrl;a.download=block.name||'arquivo';a.click();return}
  if(e.target.closest('[data-clear-file]')){e.preventDefault();Object.assign(block,{name:'',mime:'',size:0,dataUrl:''});touchItem(item);renderDrawer();return}
});
...
let draggedFolder=null;$('#folderNav').addEventListener('dragstart',e=>{const row=e.target.closest('[data-folder-id]');if(row)draggedFolder=row.dataset.folderId});$('#folderNav').addEventListener('dragover',e=>e.preventDefault());$('#folderNav').addEventListener('drop',async e=>{e.preventDefault();const target=e.target.closest('[data-folder-id]')?.dataset.folderId;if(!draggedFolder||!target||target===draggedFolder)return;const from=state.folders.findIndex(f=>f.id===draggedFolder),to=state.folders.findIndex(f=>f.id===target),[moved]=state.folders.splice(from,1);state.folders.splice(to,0,moved);state.folders.forEach((f,i)=>{f.order=i;f.updatedAt=now()});renderSidebar();for(const f of state.folders)await saveRecord('folder',f);draggedFolder=null});

// Atalhos seguros
document.addEventListener('keydown',e=>{
  const mod=e.ctrlKey||e.metaKey;
  if(e.altKey&&!e.ctrlKey&&!e.metaKey&&e.key.toLowerCase()==='n'&&state.user){e.preventDefault();createItem('blank')}
  if(mod&&e.key.toLowerCase()==='k'&&state.user){e.preventDefault();$('#searchInput').focus();$('#searchInput').select()}
  if(mod&&e.key.toLowerCase()==='s'&&state.user){e.preventDefault();const item=activeItem();if(item)saveRecord('item',item).then(()=>toast('Item sincronizado.'))}
